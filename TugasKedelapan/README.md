# Platform-Specific Features Update

## Arsitektur Singkat

`UI Composable -> Koin DI -> ViewModel/Repository -> SQLDelight & DataStore`

Platform-specific layer dipisah lewat `expect/actual` untuk:
- `DeviceInfo`
- `NetworkMonitor`
- `DatabaseDriverFactory`
- `SettingsDataStore`

## Fitur yang Ditambahkan

- Koin DI untuk seluruh app
- Device info di Settings screen
- Network status indicator di main screen
- Semua dependency diambil dari Koin

# NAMA : Abi Sholihan
# NIM  : 123140192
# SCREENSHOT APLIKASI :
## Tampilan Database Schema
1. Saat aplikasi dibuka:
   Database lokal → logic aplikasi → tampilan
2. Saat catatan diedit:
   update data → simpan ke database → refresh tampilan
3. Saat membuat catatan baru:
   input data → validasi → insert ke tabel notes → jika perlu insert relasi ke notes_folder
4. Saat menghapus catatan:
   hapus data dari notes → hapus relasi di notes_folder → refresh tampilan

## Tampilan Layar Utama
![Layar Utama](https://drive.google.com/uc?export=view&id=14cx1fNWdpa0k0UlbhPrMFSPnrB5oT3Gm)

## Tampilan Layar Tambah Catatan
![Layar Tambah Catatan](https://drive.google.com/uc?export=view&id=1guC1AaWxeV2t3i71pgjTqsWlcOgDzLs1)

## Tampilan Layar Detail Catatan
![Layar Detail Catatan](https://drive.google.com/uc?export=view&id=1QZuW-zi5DVCZoV8Omi_yhcyGXBo6fPUb)

## Tampilan Layar Edit Catatan
![Layar Edit Catatan](https://drive.google.com/uc?export=view&id=1ntq-LrXvzwkNvAMHmHDI_gfF_0zFdQPr)

## Tampilan Layar Catatan Favorit
![Layar Catatan Favorit](https://drive.google.com/uc?export=view&id=1M2Ks1sCSVI2QDZ32YmemTHePbPv5C0ku)

## Tampilan Layar Profile
![Layar Profile](https://drive.google.com/uc?export=view&id=1-SzFTKtNGuIR0izgy-u2mb4MDPTb-4_T)

## Tampilan Settings
![Settings](https://drive.google.com/uc?export=view&id=1RNuTbN7UCY77zUgVzISqkKFCwFV3LRtu)


## Offline-first
Semua data notes, pencarian, dan settings disimpan lokal melalui SQLDelight dan DataStore, jadi aplikasi tetap bisa dipakai tanpa internet.
